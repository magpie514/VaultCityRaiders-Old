...
func moveCasterStd(pos, time):
	...
	if pos == 0:
		casterSprite.moveTo(P + Vector2(-60, 0), time)
	elif pos == 1:
		casterSprite.moveTo(Vector2(500, P.y), time)
	else: return

func resetCasterPos(time): casterSprite.moveTo(casterSprite.get_global_pos(), time)

func shakeDefender(time): defenderSprite.shake(time)

func changeAnim(anim): animPlayer.play(anim)

func cameraLookDefender(): battle.cameraLookAt(defender[0], defender[1])

func _on_Timer_timeout(): finish()

func _on_Timer_timeout2(): queue_free()

func finish(): #Normal finish, continue and remove effect.
	emit_signal("actionNext")
	queue_free()

func finish_alt(): #Special finish, continue but let effect continue until timer runs out.
	emit_signal("actionNext")
